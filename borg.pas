...
begin
  ...
  EnableMenuItem(Menu,change_oep,MF_ENABLED);
  EnableMenuItem(Menu,jump_to,MF_ENABLED);
...
const
  WM_debug_ack =WM_USER+1112;
  WM_debug_init=WM_USER+1113;
  WM_debug_jmp =WM_USER+1114;
  t:lptr=(s:0;o:0);
var
  dc:HDC;
  tm:tTEXTMETRIC;
...
begin
  ...
  case msg of
   ...
   WM_debug_jmp:
    begin
      t:=plptr(lparam)^;
      scheduler.addtask(user_jumptoaddr,priority_userrequest,t,0,nil);
   ...
   WM_COMMAND:
    case LOWORD(wParam) of
     ...
     font_system:    begin options.font:=dword(systemfont);    setupfont; end;
     font_courier:   begin options.font:=dword(courierfont);   setupfont; end;
     font_courier10: begin options.font:=dword(courierfont10); setupfont; end;
     font_courier12: begin options.font:=dword(courierfont12); setupfont; end;
     font_ansi:      begin options.font:=dword(ansifont);      setupfont; end;
   ...
   WM_VSCROLL:
    case LOWORD(wParam) of
     ...
     SB_PAGEUP:
      begin
///2016
        scrll.s:=0; integer(scrll.o):=-(mainwndsize.bottom div cyChar)+1;
        if InThread
        then scheduler.addtask(scrolling,priority_userrequest,scrll,0,nil);
      end;
     SB_PAGEDOWN:
